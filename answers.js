...
information.hourly.data.forEach(x => arrayPrecipType.push(x.icon));

function willItRain(array) {
    if (array.indexOf("rain") > -1) {
        return true;
    }
    else {
        return false;
...
information.hourly.data.forEach(x => arrayPrecipTypeTwo.push(x.icon));
...
information.hourly.data.forEach(x => temperatureArrays.push(x));


var finalTempArray = temperatureArrays.map(x => x.temperature);
...
information.hourly.data.forEach(x => arrayPrecipTypeThree.push(x));
...
var rainyArray = arrayPrecipTypeThree.filter(x => x.icon === "rain");
...
dailyWeatherArray.forEach(function(x) {
    if (weatherObj[x]) {
        weatherObj[x]++;
    }
    else {
...
var busArray = infoTwo.filter(function(x) {
    if (x.email.includes(".biz")) {
        return x;
    }
})

var cityArray = busArray.map(x => x.address.city);

var uniqueArray = cityArray.forEach(function(x) {
